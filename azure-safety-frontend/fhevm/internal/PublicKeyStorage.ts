...
function assertFhevmStoredPublicKey(
  ...
  if (!("publicKeyId" in value) || typeof value.publicKeyId !== "string") {
    throw new Error(`FhevmStoredPublicKey.publicKeyId is invalid`);
  }
  if (!("publicKey" in value) || !(value.publicKey instanceof Uint8Array)) {
    throw new Error(`FhevmStoredPublicKey.publicKey is invalid`);
  }
}
...
function assertFhevmStoredPublicParams(
  ...
  if (
    !("publicParamsId" in value) ||
    typeof value.publicParamsId !== "string"
  ) {
    throw new Error(`FhevmStoredPublicParams.publicParamsId is invalid`);
  }
  if (
    !("publicParams" in value) ||
    !(value.publicParams instanceof Uint8Array)
  ) {
    throw new Error(`FhevmStoredPublicParams.publicParams is invalid`);
  }
}
